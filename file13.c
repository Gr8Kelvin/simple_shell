	...
	if (!*lenn)
	{
		...
		*buf = NULL;
		...
		if (rn > 0)
		{
			...
			rmv_comments(*buf);
			build_hist(info, *buf, info->historycount++);
			{
				*lenn = rn;
...
int get_line(inform_t *info, char **ptr, size_t *length)
{
	...
	if (sn)
		string_cat(new_p, buf + ias, kas - ias);
	else
		string_cpy(new_p, buf + ias, kas - ias + 1);
