...
/**
 * fd_cmd - find command PATH
 * @info: parameter & return struct
 * Return: void
 */
void fd_cmd(inform_t *info)
{
	...
	else
	{
		if ((inte(info) || disp_env(info, "PATH=")
					|| info->commandargs[0][0] == '/') && id_cmd(info, info->commandargs[0]))
			fk_cmd(info);
...
/**
 * fk_cmd - forks exec thread
 * @info: parameter & return struct
 * Return: void
 */
void fk_cmd(inform_t *info)
{
	pid_t kid_pid;
